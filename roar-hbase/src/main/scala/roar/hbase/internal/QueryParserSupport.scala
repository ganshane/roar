...
import roar.api.meta.{ResourceDefinitionConversions, AnalyzerCreator, ResourceDefinition, ColumnType}
...
import ResourceDefinitionConversions._
  ...
  private[roar] def createParser(fun: Option[PartialFunction[(String, String), Query]] = None) = {
    ...
    val parser = new MultiFieldQueryParser(this.defaultSearchFields, analyzer, boostProperties) {
      ...
      override def newTermQuery(term: Term): Query = {
        ...
        if (field == RoarHbaseConstants.UPDATE_TIME_FIELD_NAME) {
          val value = term.text().toInt
...
/*        } else if (field == RoarHbaseConstants.OBJECT_ID_FIELD_NAME) {
      ...
      override def newRangeQuery(field: String, part1: String, part2: String, startInclusive: Boolean, endInclusive: Boolean): Query = {
        ...
        } else if (field == RoarHbaseConstants.UPDATE_TIME_FIELD_NAME) {
